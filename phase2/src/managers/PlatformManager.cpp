...
std::unique_ptr<Platform> PlatformManager::createRandom(float y) {
    ...
    if (choice == 1) {
        return std::make_unique<MovingPlatform>(textures.get("moving_platform"), pos,
                                                Constants::MOVING_PLATFORM_SPEED * speedMultiplier,
...
void PlatformManager::checkCollisions(Player& player, float previousPlayerBottom, SoundManager* soundManager) {
...
const std::vector<std::unique_ptr<Platform>>& PlatformManager::getPlatforms() const {
    return platforms;
}

std::vector<sf::FloatRect> PlatformManager::getAllBounds() const {
...
void PlatformManager::setSpeedMultiplier(float multiplier) {
